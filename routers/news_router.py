...
from database.schemas.news_schema import news_schema
...
@news_router.post("/", response_model=dict)
...
async def get_news_by_id(news_id: str):
    ...
    if not news:
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="News not found")
...
async def patch_news(news_id: str, updated_fields: dict):
    ...
    result = db_client.local.news.update_one({"_id": ObjectId(news_id)}, {"$set": updated_fields})
